import { MissingAuthHeaderError, DEFAULT_AUTH_TOKEN_TTL, ClientInitiatedHandshake, type VerifyPeer } from '@libp2p/http-peer-id-auth'
...
export class PeerIdAuth implements Middleware {
  ...
  processResponse (resource: URL | Multiaddr[], opts: MiddlewareOptions, response: Response): void {
    ...
    if (token?.peerId != null) {
      ...
      // the property is read-only so we can't just assign it
